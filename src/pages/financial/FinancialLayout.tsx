...
export default function FinancialLayout() {
  ...
  return (
    <div className="flex bg-gray-50 h-[calc(100vh-48px)] overflow-hidden">
      {/* Secondary Sidebar */}
      <aside className="w-56 bg-gray-50 border-r border-gray-200 flex flex-col shrink-0 overflow-y-auto hidden md:flex">
        <div className="p-4 py-5 flex items-center justify-between pointer-events-none">
        ...
        <nav className="flex-1 px-3 pb-6 space-y-0.5">
          <NavLink to="/financial/dashboard" className={({ isActive }) => `flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors ${isActive ? 'bg-gray-200 text-gray-900 font-bold' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}>
          ...
          {MENU.map(m => (
            <NavLink key={m.to} to={m.to} className={({ isActive }) => `flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors ${isActive ? 'bg-gray-200 text-gray-900 font-bold' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}>
          ...
          <div className="pt-3">
            ...
            {boletosOpen && (
              <div className="pl-6 mt-0.5 space-y-0.5">
                {BOLETOS.map(m => (
                  <NavLink key={m.to} to={m.to} className={({ isActive }) => `flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${isActive ? 'bg-gray-200 text-gray-900 font-bold' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-800'}`}>
      ...
      {/* Main Content Area */}
      <main className="flex-1 bg-white overflow-y-auto relative flex flex-col">
        <Outlet />
      </main>
    </div>
  )
